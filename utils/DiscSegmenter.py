#### CODE EXTRACTED FROM https://pvbm.readthedocs.io/en/latest/DiscSegmenter.html
...
class DiscSegmenter:
    ...
    def post_processing(self, segmentation, max_roi_size, image_type: str = "ODC"):
        """
        ...
        try:
            ...
            if image_type == "ODC": #here, roi for Df computation is either 0.5-2.0 OD diameters
                cv2.circle(roi, center, radius * 4, (0, 255, 0), -1)
                cv2.circle(roi, center, int(radius * 2), (0, 0, 0), -1)
            
            elif image_type == "MC": 
                #roi for Df Dmac is defined as the region with diameter 5 times the optic disc diameter, 
                #with its medial border displaced 1/4 of a disc diameter nasally to the nasal border of the optic disc.

                #1. Know if the OD is on the left or right side of the image
                img_center_x = segmentation.shape[1] // 2
                if center[0] < img_center_x: #OD on the left side, then nasal border is on the left
                    nasal_border_x = int(center[0] - radius - 0.25 * (2 * radius)) #center - radius = nasal border; move 0.25*diameter to the left
                    roi_center = (nasal_border_x + int(5 * radius), center[1]) #center of the roi is 2.5*diameter to the right of the nasal border
                
                else: #OD on the right side, then nasal border is on the right
                    nasal_border_x = int(center[0] + radius + 0.25 * (2 * radius)) #center + radius = nasal border; move 0.25*diameter to the right
                    roi_center = (nasal_border_x - int(5 * radius), center[1]) #center of the roi is 2.5*diameter to the left of the nasal border

                #2. Draw the roi
                cv2.circle(roi, roi_center, int(5 * radius), (0, 255, 0), -1)
                
                #Remove the optic disc area from the roi
                cv2.circle(roi, center, radius, (0, 0, 0), -1)
            else:
